...
(function ($) {
    $.fn.mtimer = function (opts) {
        ...
        var defaults = {
            ...
            option = $.extend(defaults, opts), // 合并配置
            input = $(this), // 配置this
            itemHeight = 48, // 一个元素的高度，方便后面计算滚动到哪个元素
        ...
        var picker = {
            ...
            init: function () {
                ...
                var elHeight = itemHeight,
                    dateScroll = new IScroll('.mt_date', {
                        snap: 'li',
                        probeType: 2,
                        tap: true
                    });
                ...
                input.on('tap', function () {
                    if (container.hasClass('show')) _this.hidePanel();
                ...
                $('.mt_ok', container).on('tap', function () {
                    var date = mpDate.find('.' + selectedClass).data('date'),
                        time = mpTime.find('.' + selectedClass).data('time');
                    input.val(date + ' ' + time);
                    _this.hidePanel();
            ...
            checkDate: function (el) {
                ...
                if (target.length) this.dateScroll.scrollToElement(target[0]);
            },
