    ...
    $.fn.mdater = function (config) {
        ...
        var mdater = {
            ...
            getDateStr: function(y, m, d) {
                var dayStr = '';
                // 计算一号是星期几，补上上上个月的末尾几天
                var week = F.getWeekInMonth(y, m),
                    lastMonthDays = F.getDaysInMonth(y, m-1);
                for (var i = week - 1; i >= 0; i--) {
                    dayStr += '<li class="prevdate" date-day="' + (lastMonthDays - i) + '">' + (lastMonthDays - i) + '</li>';
                }
                // 补上当月所有天数
                ...
                for (var i = endDay + 1; i <= currentMonthDays; i++) {
                    dayStr += '<li class=" disabled" data-day="' + i + '">' + i + '</li>';
                }
                ...
                if (nextMonthStartWeek !== 0) {
                    for (var i = 1; i <= 7 - nextMonthStartWeek; i++) {
                        dayStr += '<li class="nextDate" date-day="' + i + '">' + i + '</li>';
                    }
                }
                return dayStr;
            },
